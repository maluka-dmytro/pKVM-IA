...
static int __vcpu_create(struct kvm *kvm, struct kvm_vcpu *vcpu, struct fpstate *fps)
{
	...
	/*
	 * Load guest vCPU to reset. The guest vCPU should be put before
	 * switching back to the host vCPU to make sure the vcpu state is not
	 * cached on this CPU as this guest vCPU may be loaded on another CPU
	 * later by the host via the PV interface.
	 */
...
static int pkvm_set_msr(struct kvm_vcpu *vcpu, u32 index, u64 data)
{
	if (pkvm_is_protected_vcpu(vcpu)) {
		...
		default:
			 ...
			 */
			ret = kvm_msr_read(vcpu, index, &cur_data);
			if (ret)
				return ret;
			if (data == cur_data)
				return 0;
				     ...
				     struct pkvm_memcache *mc)
{
	...
	aligned_size = PAGE_ALIGN(sizeof(struct kvm_cpuid_entry2) * new_nent);
...
static void pkvm_load_mmu_pgd(struct kvm_vcpu *vcpu)
{
					   ...
					   union pkvm_hc_data *in, union pkvm_hc_data *out)
{
	...
	if (!vcpu || hvcpu != this_cpu_read(host_vcpu))
		return -EINVAL;
	...
	case __pkvm__set_cr4:
		...
		break;
	case __pkvm__post_set_cr3:
		kvm_x86_call(post_set_cr3)(vcpu, pkvm_hc_input1(hvcpu));
		break;
	case __pkvm__set_cr0:
...
int pkvm_start_secondary_vcpu(struct kvm *kvm, u32 apic_id, unsigned long start_ip)
{
	...
	for (i = 0; i < kvm->created_vcpus; i++) {
		...
		/*
		 * Update sipi_vector before updating mp_state, i.e. before
		 * allowing the vCPU to run. Pairs with smp_load_acquire()
		 * in pkvm_vcpu_run().
		 */
