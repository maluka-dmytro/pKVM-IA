...
int pkvm_emulate_hypercall(struct kvm_vcpu *vcpu)
{
	...
	case PKVM_GHC_SHARE_MEM:
		...
		/*
		 * May require additional pages in order to split a huge mapping into
		 * smaller ones. Assume the worst case.
		 */
		...
		if (vcpu->arch.pkvm.guest_mmu_memcache.count < memcache_refill_size) {
			/*
			 * If not enough pages in the memcache to handle mapping, request
			 * should be forwarded to the host for refilling the memcache with
			 * the needed amount of pages, after which the instruction will be
			 * re-issued (don't skip the instruction).
			 */
...
/*
 * Returns true if pKVM should skip the instruction after the given hypercall
 * was handled by the host.
 *
 * In contrast, if a hypercall is handled solely by pKVM without host's help,
 * the instruction is always skipped unconditionally.
 */
bool pkvm_hypercall_need_skip_instruction(unsigned long nr)
{
	...
	case PKVM_GHC_SHARE_MEM:
		/*
		 * Let the guest re-issue the instruction to retry sharing
		 * after the host has refilled the memcache. See comments in
		 * pkvm_emulate_hypercall().
		 */
		return false;
	default:
		return true;
