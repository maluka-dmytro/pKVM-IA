...
static void *admit_host_page(void *arg)
{
	 ...
	 */
	if (pkvm_host_donate_hyp(__pkvm_pa(page), PAGE_SIZE, false)) {
		WARN_ON(1);
				...
				unsigned long size, u64 prot, void *arg)
{
	...
	ret = pkvm_pgtable_map(&pkvm_vm->mmu, gpa, hpa, size, prot, NULL);
	if (ret)
		return ret;
...
void pkvm_guest_mmu_free_memcache(struct pkvm_vcpu *pkvm_vcpu)
{
	...
	struct pkvm_memcache *teardown_mc = &shared_pkvm->guest_mmu_teardown_mc;
	void *addr;
			   ...
			   unsigned long hpa, unsigned long size)
{
	...
	if (WARN_ON_ONCE(!pkvm_is_protected_vm(&pkvm_vm->kvm)))
		return -EPERM;
	...
	ret = check_page_owner_and_state(&pkvm_vm->mmu, gpa, size,
					 PKVM_ID_HOST, PKVM_PAGE_NONE);
	if (ret)
		goto unlock;
...
/**
 * pkvm_host_share_guest() - Un-share host pages with a guest.
 * @kvm:	Guest VM to unshare the host pages with.
 * @gpa:	Address of the guest physical address region to unshare.
 * @size:	Size of the guest physical address region to share.
 *
			  ...
			  unsigned long size)
{
	...
	if (ret) {
		...
		for_each_contig_range(&pkvm_vm->mmu, gpa, size,
