...
static void pkvm_vm_destroy(struct kvm *kvm)
{
	...
	/*
	 * It would be nice to do this in generic code in kvm_mmu_uninit_vm(),
	 * not here. But kvm_mmu_uninit_vm() is called too early, when pKVM
	 * has not released guest pages to the host yet.
	 */
...
static u64 pkvm_get_segment_base(struct kvm_vcpu *vcpu, int seg)
{
	...
	if (!pkvm_segment_cache_test(vmx, seg, SEG_FIELD_BASE)) {
		if (pkvm_hypercall_out(get_segment_base, &out, seg))
			*p = 0;
		else
...
static void pkvm_set_segment(struct kvm_vcpu *vcpu, struct kvm_segment *var, int seg)
{
	...
	WARN_ON(pkvm_hypercall_in(set_segment, &in));
}
...
static fastpath_t pkvm_vcpu_run(struct kvm_vcpu *vcpu, u64 run_flags)
{
	...
	if (pkvm_hypercall_out(vcpu_run, &out, force_immediate_exit) != 0) {
		vmx->fail = 1;
...
static u32 pkvm_get_interrupt_shadow(struct kvm_vcpu *vcpu)
{
	...
	pkvm_hypercall_out(get_interrupt_shadow, &out);
...
static bool pkvm_get_nmi_mask(struct kvm_vcpu *vcpu)
{
	...
	return pkvm_hypercall_out(get_nmi_mask, &out) ? false : out.get_nmi_mask.data;
}
...
static void pkvm_vcpu_after_set_cpuid(struct kvm_vcpu *vcpu)
{
	...
	if (pkvm_hypercall_out(vcpu_after_set_cpuid, &out, __pa(entries)))
