				  ...
				  struct perf_data **dump, unsigned long *size)
{
	...
	for (cpu = 0, pervcpu = *dump;
		 ...
		 * means all the host's perf data are already printed.
		 */
				   ...
				   struct perf_data **dump, unsigned long *size)
{
	...
	for (pervcpu = *dump;
		...
		/*
		 * The trace data from the pKVM hypervisor is grouped by VM.
		 * If the vm_handle in this perf data is not for the guest VM,
		 * it means all the guest VM's perf data are already printed.
		 */
