...
void vmx_update_exception_bitmap(struct kvm_vcpu *vcpu)
{
...
#ifdef __PKVM_HYP__
	/*
	 * For simplicity, not to intercept #PF/#UD/#GP for pVM to avoid
	 ...
	 * for #GP). The handling #BP needs to exit to the user space VMM in the
...
static int vmx_alloc_ipiv_pid_table(struct kvm *kvm)
{
	...
	/*
	 * Although the contents in pid_table is not secret which is constructed
	 * following the SDM, donate the pid_table pages to the pKVM hypervisor
	 * as the host doesn't need to access these pages until the guest is
	 * destroyed.
	 */
...
static int handle_invalid_guest_state(struct kvm_vcpu *vcpu)
{
	 ...
	 * guest state should be valid, unless there is a code bug.
	 */
	BUG_ON(1);
#endif
}
...
void vmx_set_virtual_apic_mode(struct kvm_vcpu *vcpu)
{
	 ...
	 * instruction thus host cannot emulate xapic mode for a pVMs. Not to
	 * use xapic mode for a pVMs.
	 */
	if (pkvm_is_protected_vcpu(vcpu) &&
					...
					unsigned int flags)
{
	 ...
	 * host.
	 * The CPU can run the pKVM hypervisor doesn't have X86_BUG_L1TF and
	 * X86_BUG_MMIO_STALE_DATA which is guaranteed by the deprivilege code,
	 * thus it is not necessary to apply the corresponding mitigation.
	 */
#ifndef __PKVM_HYP__
...
int vmx_vm_init(struct kvm *kvm)
{
	 ...
	 * since pkvm_has_unmitigated_cpu_bugs() doesn't currently mitigate L1TF
	 * and thus would fail pKVM initialization if L1TF was present. Thus it
	 * is not necessary for pKVM to check the L1TF mitigation.
	 */
#ifndef __PKVM_HYP__
					...
					struct kvm_vcpu *shared_vcpu)
{
	...
	case EXIT_REASON_MSR_WRITE:
		WARN_ON_ONCE(kvm_complete_insn_gp(vcpu,
				xchg(&to_pkvm_vcpu(vcpu)->host_emulated_msr_err, 0)) != 1);
		break;
	default:
		break;
	}
...
static void pkvm_vmx_update_vcpu_state_from_host(struct pkvm_vcpu *pkvm_vcpu)
{
...
static void pkvm_vmx_share_vcpu_state_with_host(struct pkvm_vcpu *pkvm_vcpu)
{
	struct kvm_vcpu *shared_vcpu = pkvm_vcpu->shared_vcpu;
	...
	if (!pkvm_is_protected_vcpu(vcpu)) {
		if (unlikely(WARN_ON(vmx->rmode.vm86_active))) {
