...
#else
		/*
		 * The error KVM_MSR_RET_UNSUPPORTED may happen if an MSR should
		 * be emulated by the host but not the pKVM hypervosr. In this
		 * case, go back to the host.
		 *
		 * If an MSR should be emulated by the pKVM hypervisor but not
		 * the host, the errors (not just KVM_MSR_RET_UNSUPPORTED but
		 * also the other errors) should be handled inside the pKVM as
		 * the host doesn't have emulation for such MSR.
		 */
		if (r == KVM_MSR_RET_UNSUPPORTED &&
		    pkvm_host_has_emulated_msr(vcpu->kvm, msr))
			return 0;
#endif
...
int pkvm_emulate_hypercall(struct kvm_vcpu *vcpu)
{
	...
	case PKVM_GHC_SHARE_MEM:
		...
		if (vcpu->arch.pkvm.guest_mmu_memcache.count < memcache_refill_size) {
			 ...
			 * after which the instruction will be re-issued (so the vendor
			 * pKVM code is expected to avoid skipping the instruction in
			 * this case).
			 */
