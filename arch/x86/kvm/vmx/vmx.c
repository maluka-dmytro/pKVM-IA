...
static void pkvm_vmx_update_vcpu_state_from_host(struct kvm_vcpu *vcpu)
{
	...
	if (pkvm_is_protected_vcpu(vcpu) &&
	    ...
	    !vmx_get_exit_reason(vcpu).failed_vmentry &&
	    !vmx->fail)
		update_protected_vcpu_state(vcpu, to_pkvm_vcpu(vcpu)->shared_vcpu);
}
...
static void pkvm_vmx_share_vcpu_state_with_host(struct kvm_vcpu *vcpu)
{
	...
	if (!pkvm_is_protected_vcpu(vcpu)) {
		/*
		 * Share the npVM's AR reg of CS/SS for optimization purposes as
		 * these are more frequently used by the host to emulate
		 * instructions or scheduling a guest state non-protected vCPU
		 * at the run time so that the host doens't need to send
		 * additional PV interfaces to retrieve them. No need to share
		 * for a pVM as the host cannot emulate instructions for it.
		 */
