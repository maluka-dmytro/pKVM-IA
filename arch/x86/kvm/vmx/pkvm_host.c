...
static void pkvm_vcpu_reset(struct kvm_vcpu *vcpu, bool init_event)
{
	...
	if (!vcpu->arch.guest_state_protected)
		pkvm_hypercall(vcpu_reset, init_event);
	...
	if (pkvm_is_protected_vcpu(vcpu)) {
		 ...
		 * instruction which is not supported if the gust is a pVM as
...
static int pkvm_get_msr(struct kvm_vcpu *vcpu, struct msr_data *msr_info)
{
	...
	if (!vcpu->arch.guest_state_protected) {
		...
		ret = pkvm_hypercall_inout(get_msr, &inout);
		msr_info->data = inout.msr.data;
...
static u64 pkvm_get_segment_base(struct kvm_vcpu *vcpu, int seg)
{
	...
	if (!pkvm_segment_cache_test(vmx, seg, SEG_FIELD_BASE)) {
		if (pkvm_hypercall_inout(get_segment_base, &inout))
			*p = 0;
		else
			*p = inout.seg_val.base;
...
static void pkvm_get_segment(struct kvm_vcpu *vcpu, struct kvm_segment *var, int seg)
{
	...
	if (!pkvm_segment_cache_test(vmx, seg, SEG_FIELD_SEL) ||
		...
		WARN_ON(pkvm_hypercall_inout(get_segment, &inout));

		pkvm_cache_segment(vmx, &inout.seg_val, seg);
	}
...
static void pkvm_set_segment(struct kvm_vcpu *vcpu, struct kvm_segment *var, int seg)
{
	...
	KVM_BUG_ON(pkvm_hypercall_inout(set_segment, &inout), vcpu->kvm);
}
				...
				bool set, bool idt)
{
	...
	if (set) {
		...
		if (idt)
			pkvm_hypercall_inout(set_idt, &in);
		else
			pkvm_hypercall_inout(set_gdt, &in);
	} else {
		union pkvm_hc_data out = { 0 };

		if (idt)
			pkvm_hypercall_inout(get_idt, &out);
		else
			pkvm_hypercall_inout(get_gdt, &out);
		dt->size = out.desc.size;
		dt->address = out.desc.address;
	}
}
...
static void pkvm_cache_reg(struct kvm_vcpu *vcpu, enum kvm_reg reg)
{
	...
	if (KVM_BUG_ON(pkvm_hypercall_inout(cache_reg, &inout), vcpu->kvm))
		return;

	switch (reg) {
	case VCPU_REGS_RSP:
		vcpu->arch.regs[VCPU_REGS_RSP] = inout.rsp;
		break;
	case VCPU_REGS_RIP:
		vcpu->arch.regs[VCPU_REGS_RIP] = inout.rip;
		break;
	case VCPU_EXREG_PDPTR: {
		struct kvm_mmu *mmu = vcpu->arch.walk_mmu;

		mmu->pdptrs[0] = inout.pdptrs[0];
		mmu->pdptrs[1] = inout.pdptrs[1];
		mmu->pdptrs[2] = inout.pdptrs[2];
		mmu->pdptrs[3] = inout.pdptrs[3];
		break;
	}
	case VCPU_EXREG_CR0:
		vcpu->arch.cr0 = inout.cr0;
		break;
	case VCPU_EXREG_CR3:
		vcpu->arch.cr3 = inout.cr3;
		break;
	case VCPU_EXREG_CR4:
		vcpu->arch.cr4 = inout.cr4;
		break;
	default:
...
static unsigned long pkvm_get_rflags(struct kvm_vcpu *vcpu)
{
	...
	if (!kvm_register_is_available(vcpu, VCPU_EXREG_RFLAGS)) {
		...
		if (vcpu->arch.guest_state_protected) {
			...
			pkvm_hypercall_inout(get_rflags, &out);
			vmx->rflags = out.rflags;
		}
	}
...
{
	union pkvm_hc_data inout = {
		.force_immediate_exit = run_flags & KVM_RUN_FORCE_IMMEDIATE_EXIT,
	};
	struct vcpu_vmx *vmx = to_vmx(vcpu);
	unsigned long reqs_to_host;
	fastpath_t exit_fastpath;

	trace_kvm_entry(vcpu, inout.force_immediate_exit);
	...
	if (pkvm_hypercall_inout(vcpu_run, &inout) != 0) {
		vmx->fail = 1;
		guest_state_exit_irqoff();
		return EXIT_FASTPATH_NONE;
	}

	reqs_to_host = inout.reqs_to_host;
	...
	if (unlikely(vmx_get_exit_reason(vcpu).full == 0xdead)) {
		vmx->fail = 1;
	} else if ((u16)vmx_get_exit_reason(vcpu).basic == EXIT_REASON_EXCEPTION_NMI &&
...
{
	if (!vcpu->arch.guest_state_protected)
		pkvm_hypercall(set_interrupt_shadow, mask);
}
...
static u32 pkvm_get_interrupt_shadow(struct kvm_vcpu *vcpu)
{
	...
	pkvm_hypercall_inout(get_interrupt_shadow, &out);

	return out.intr_shadow;
}
...
static void pkvm_cancel_injection(struct kvm_vcpu *vcpu)
{
	...
	pkvm_hypercall(cancel_injection);
...
static int pkvm_vcpu_realloc_fpstate(struct kvm_vcpu *vcpu)
{
	...
	inout.fpstate_gpa = __pa(fps);
	inout.fpstate_size = fpsize;
	ret = pkvm_hypercall_inout(vcpu_add_fpstate, &inout);
	if (ret)
...
static void pkvm_vcpu_after_set_cpuid(struct kvm_vcpu *vcpu)
{
	...
	inout.cpuid_gpa = __pa(entries);
	if (pkvm_hypercall_inout(vcpu_after_set_cpuid, &inout))
...
{
	pkvm_hypercall(write_tsc_offset, vcpu->arch.tsc_offset);
}

static void pkvm_write_tsc_multiplier(struct kvm_vcpu *vcpu)
{
	pkvm_hypercall(write_tsc_multiplier, vcpu->arch.tsc_scaling_ratio);
}
