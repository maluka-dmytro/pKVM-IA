...
static int pkvm_vcpu_add_fpstate(struct pkvm_vcpu *pkvm_vcpu,
...
static int pkvm_setup_mce(struct pkvm_vcpu *pkvm_vcpu, u64 mcg_cap)
{
	return kvm_vcpu_x86_setup_mce(&pkvm_vcpu->vcpu, mcg_cap);
}

static void update_vcpu_state_from_host(struct pkvm_vcpu *pkvm_vcpu)
{
	...
	pkvm_x86_call(update_vcpu_state_from_host)(pkvm_vcpu);
}

static void share_vcpu_state_with_host(struct pkvm_vcpu *pkvm_vcpu)
{
	struct kvm_vcpu *shared_vcpu = pkvm_vcpu->shared_vcpu;
	struct kvm_vcpu *vcpu = &pkvm_vcpu->vcpu;
	...
	pkvm_x86_call(share_vcpu_state_with_host)(pkvm_vcpu);
}

static int pkvm_vcpu_run(struct pkvm_vcpu *pkvm_vcpu, bool force_immediate_exit,
			 unsigned long *reqs_to_host)
{
	int ret;

	update_vcpu_state_from_host(pkvm_vcpu);

	ret = pkvm_vcpu_enter_guest(pkvm_vcpu, force_immediate_exit);

	share_vcpu_state_with_host(pkvm_vcpu);
					   ...
					   union pkvm_hc_data *out)
{
	...
	case __pkvm__vcpu_after_set_cpuid:
		...
		break;
	case __pkvm__vcpu_add_fpstate:
		ret = pkvm_vcpu_add_fpstate(pkvm_vcpu, pkvm_host_gpa_to_phys(in->fpstate_gpa),
					    (size_t)in->fpstate_size, &out->memcache);
		break;
	case __pkvm__write_tsc_offset:
		pkvm_write_tsc_offset(pkvm_vcpu, (u64)in->val1);
		break;
	...
	case __pkvm__setup_mce:
		ret = pkvm_setup_mce(pkvm_vcpu, (u64)in->val1);
		break;
